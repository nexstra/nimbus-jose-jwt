...
public class Ed25519SignVerifyTest extends TestCase {
	...
	public void testInvalidSignerInstantiation() throws GeneralSecurityException {
		...
		try {
			new Ed25519Signer(k2.toPublicJWK());
	...
	public void testInvalidVerifierInstantiation() throws Exception {
		...
		try {
			new Ed25519Verifier(k1.toPublicJWK());
			fail("should fail if key provided with wrong curve");
		...
		try {
			new Ed25519Verifier(k2);
			fail("should fail if private key provided");

		} catch (JOSEException e) {
			// Passed
		}

		new Ed25519Verifier(k2.toPublicJWK());
	}
	...
	public void testInvalidHeader() throws GeneralSecurityException {
		...
		try {
			// Signature is invalid, but should throw instead of returning false
			verifier.verify(h2, new byte[] {1,2,3}, Base64URL.encode(new byte[64]));
			fail("should fail with alg ES256");
	...
	public void testSignVerifyRandom() throws Exception {
	...
	public void testRfc8032Vectors() throws Exception {
		...
		for (byte[][] testVector : testVectors) {
			...
			assertEquals(testVector.length, 4);
			byte[] sk = testVector[0];
			Base64URL skB64 = Base64URL.encode(sk);
			byte[] pk = testVector[1];
			Base64URL pkB64 = Base64URL.encode(pk);
			byte[] m = testVector[2];
			byte[] s = testVector[3];
			Base64URL sB64 = Base64URL.encode(s);

			OctetKeyPair okp = new OctetKeyPair.Builder(Curve.Ed25519, pkB64).
				d(skB64).
				build();

			Ed25519Signer signer = new Ed25519Signer(okp);
			Assert.assertArrayEquals(signer.sign(h, m).decode(), s);

			Ed25519Verifier verifier = new Ed25519Verifier(okp.toPublicJWK());
			assertTrue(verifier.verify(h, m, sB64));
			assertFalse(verifier.verify(h, m, Base64URL.encode(new byte[64])));

			if (m.length > 0) {
				assertFalse(verifier.verify(h, new byte[0], sB64));
			} else {
				assertFalse(verifier.verify(h, new byte[64], sB64));
			}
		}
	}
	...
	public void testRfc8037Example() throws Exception {
		...
		o.sign(new Ed25519Signer(okp));

		// Note: this test might fail due to harmless changes in how the object is serialized
		String jws = o.serialize();
		assertEquals(jws, "eyJhbGciOiJFZERTQSJ9.RXhhbXBsZSBvZiBFZDI1NTE5IHNpZ25pbmc." +
			"hgyY0il_MGCjP0JzlnLWG1PPOt7-09PGcvMg3AIbQR6dWbhijcNR4ki4iylGjg5BhVsPt9g7sVvpAr_MuM0KAg");

		assertTrue(JWSObject.parse(jws).verify(new Ed25519Verifier(okp.toPublicJWK())));
	}
	...
	public void testCritParamDeferral()
		...
		jwsObject.sign(new Ed25519Signer(okp));
